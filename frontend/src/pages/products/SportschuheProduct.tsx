...
// import { productsSportschuhe } from '../../data';
...
`;
const SportschuheProduct:React.FC = () => {
    const dispatch = useAppDispatch();
    const allProducts = useAppSelector((state)=>state.products.allProducts);
    useEffect(()=>{
        dispatch(getAllProducts());
    }, [dispatch])
    const productsSportschuhe = allProducts.filter((item)=>{
        return item.categories.includes("Sportschuhe");
    })
    const [currentPage, setCurrentPage] = useState(1);
    const [productsPerPage] = useState(12);
    const lastIndex = currentPage * productsPerPage;
    const firstIndex = lastIndex - productsPerPage;
    const currentProducts = productsSportschuhe.slice(firstIndex, lastIndex);
    useEffect(() => {
  ...
  return (
    <Container>
      ...
      <Pagination
        total={productsSportschuhe.length}
...
export default SportschuheProduct
